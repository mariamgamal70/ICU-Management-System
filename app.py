...
def Admin_Add_Receptionist():
    ...
    try:
        ...
        val = (SSN, recepID, FirstName, LastName, Email, Salary,
               Gender, formatted_date, PhoneNumber, Address, Experience)
...
@app.route('/AdminNurse', methods=["GET"])
def AdminNurse():
    if request.method == "GET":
        mycursor.execute(
            "SELECT NurseID,Fname,Lname,Sex, TIMESTAMPDIFF(YEAR, Birthdate, CURDATE()) AS Age,Speciality,StartShift,EndShift FROM nurse ORDER BY NurseID ")
    # row_headers = [x[0] for x in mycursor.description]
    nurse_data = mycursor.fetchall()
    Nurse_DATA = {
        'records': nurse_data
    }
    return render_template("/Admin/AdminNurse.html", Nurse_data=Nurse_DATA)


@app.route('/Admin_Add_Nurse', methods=["POST", "GET"])
def Admin_Add_Nurse():
    if request.method == "GET":
        return render_template("/Admin/Admin_Add_Nurse.html")
    ...
    try:
        ...
        sql = "INSERT INTO nurse (Nurse_SSN,NurseID,Speciality,Fname,Lname,email,Salary, Sex, Birthdate, Phone,Address,Experience) VALUES(%s, %s, %s, %s, %s, %s, %s, %s,%s,%s,%s,%s)"
        val = (SSN, nurseID, Speciality, FirstName, LastName, Email,
               Salary, Gender, formatted_date, PhoneNumber, Address, Experience)
        mycursor.execute(sql, val)
        """
       sql="INSERT INTO user(UserID,Username,Password,Permission,email,Doctor_DoctorSSN)"
       val=(DoctorID,FirstName,Password,'Doctor',Email,SSN)
       ...
       """
        mydb.commit()
        return redirect(url_for('AdminNurse'))
...
def AddDr():
    ...
    try:
        ...
        mycursor.execute(sql, val)
        sql = "INSERT INTO user(UserID,Username,Password,Permission,email,Doctor_DoctorSSN)"
        val = (DoctorID, FirstName+' '+LastName,
               Password, "Doctor", Email, SSN)
        mycursor.execute(sql, val)
        """
    ...
    if request.method == "POST":
        if "uploadlab" in request.form:
            ...
            val = (selected_radio, file_contents,
                   request.form.get('dataissued'), 1)
            mycursor.execute(
                "INSERT INTO patientscans(Type,PatientScans,DataIssued,Patient_PSSN,Flag) VALUES (%s, %s, %s, (SELECT PSSN FROM patient WHERE PatientID=%s),'Checked')", val)
            mydb.commit()
            return redirect('/patientlabsandscans')
    mycursor.execute(
...
def dailyassessment(patient_id):
    if request.method == "POST":
        val1 = ((request.form.get('consciousness')), (request.form.get('pupils')), (request.form.get('skin')), (request.form.get('bloodpressure')), (request.form.get('bloodglucose')), (request.form.get('respiratoryrate')),
                (request.form.get('oxygensaturation')), (request.form.get('heartrate')), (request.form.get('painlevel')), (request.form.get('ivaccess')), (request.form.get('ivaccessdate')), (request.form.get('heparin')), (patient_id))
        ...
        mycursor.execute(
            'UPDATE patient SET Blood_Group=%s WHERE PatientID=%s', val2)
...
def prescriptiontable():
    mycursor.execute(
        'SELECT medicine_id,medicine_name,Dosage,Frequency,StartDate,EndDate from prescribed_medication join patient on Patient_PSSN=PSSN WHERE PatientID=%s', ([1]))
...
def administer_in_prescription_table(medicineid):
    mycursor.execute(
        'INSERT INTO medicine_prescription_timestamps(medicine_prescription_id,timestamps) values(%s, CURRENT_TIMESTAMP)', [medicineid])
    mydb.commit()
...
def prescriptiontimestamps(medicineid):
    mycursor.execute(
        'SELECT COUNT(timestamps),medicine_prescription_id,medicine_name from medicine_prescription_timestamps join prescribed_medication on medicine_prescription_id=medicine_id WHERE medicine_prescription_id=%s GROUP BY medicine_prescription_id', [medicineid])
...
def patientRecord():
    ...
    result = {}
    mycursor.execute(
        'SELECT FName,MName,LName,Sex,PSSN,Address,email,Phone,Emergency_Contact,Birthdate,Insurance_Status,PatientID from patient join patientrecord on PSSN=Patient_PSSN where patientid=%s', ([1]))
    patient = mycursor.fetchone()
    ...
    mycursor.execute(
        'SELECT TIMESTAMPDIFF (YEAR, Birthdate, CURDATE()) from patient AS age')
    patient = mycursor.fetchone()
...
def receptionistHome():
    result = {}
    mycursor.execute(
        'SELECT Fname,Lname,Receptionist_SSN,Sex,ReceptionistID from receptionist where receptionistID=%s', ([1]))
...
def manage_records():
    if request.method == 'POST':
        ...
        mycursor.execute(
            "SELECT RecordID,FName,LName,Date_Admitted from patient join patientrecord on PSSN=Patient_PSSN WHERE ReportID=%s", (ReportID))
        ...
        mycursor.execute(
            "SELECT RecordID,FName,LName,Date_Admitted from patient join patientrecord on PSSN=Patient_PSSN WHERE PatientID=%s", ([1]))
        recordinfo = mycursor.fetchall()
        data = {
            'recordinfo': recordinfo
        }
...
def editRecord(id):
    if request.method == 'POST':
        ...
        val1 = ((FirstName), (MiddleName), (LastName), (Gender), (PatientID), (SSN), (formatted_date),
                (Address), (Email), (PhoneNumber), (EmergencyContact), (AssignedDoctorSSN), (AssignedNurseSSN), (1))
        ...
        mycursor.execute(
            'UPDATE patientrecord SET RecordID=%s,Insurance_Status=%s,Patient_Pssn=%s', val2)
        mydb.commit()
...
def getdoctordata():
    ...
    result = {}
    mycursor.execute(
        "SELECT Fname, Lname, Birthdate, DoctorSSN, Sex, Doctor_ID from doctor where Doctor_ID = %s", ([14]))
...
def receptionistHomePage():
    ...
    result = {}
    mycursor.execute(
        'SELECT Fname,Lname,Receptionist_SSN,Sex,ReceptionistID from receptionist where receptionistID=%s', ([39]))
    receptionist = mycursor.fetchone()
...
def admit(file_id):
    if request.method == "POST":
        ...
        if "submit1" in request.form:
            ...
            mycursor.execute(
                "INSERT INTO prescribed_medication (medicine_id,medicine_name,Dosage, Frequency, StartDate, EndDate, Specifications,Doctor_DoctorSSN,Patient_PSSN) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", val)
            mydb.commit()
...
def R_AddRecord():
    if request.method == 'GET':
        ...
        val = (FirstName, MiddleName, LastName, Gender, PatientID, SSN, formatted_date,
               Address, Email, PhoneNumber, EmergencyContact, AssignedDoctorSSN, AssignedNurseSSN)
        mycursor.execute(sql, val)
